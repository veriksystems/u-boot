...
static struct PDMA_txdesc *tx_ring0;
...
static void  rt2880_eth_halt(struct eth_device* dev);
...
#if !defined (RALINK_MDIO_ACCESS_FUN) || !defined (RALINK_EPHY_INIT)
...
#endif // RALINK_EPHY_INIT ||  RALINK_MDIO_ACCESS_FUN //
...
#define MT7628_P0_EPHY_AIO_EN   (1<<16)
	...
	i = RALINK_REG(RALINK_AGPIOCFG_REG);
	...
	i = i & ~(MT7628_EPHY_EN);
#endif
	RALINK_REG(RALINK_AGPIOCFG_REG) = i;

	debug("RESET MT7628 PHY!!!!!!");
	// reset phy
	i = RALINK_REG(RALINK_RSTCTRL_REG);
	i = i | RSTCTRL_EPHY_RST;
	RALINK_REG(RALINK_RSTCTRL_REG) = i;
	i = i & ~(RSTCTRL_EPHY_RST);
	RALINK_REG(RALINK_RSTCTRL_REG) = i;
#if defined (CONFIG_EPHY_LED_ON)
...
static int rt2880_eth_recv(struct eth_device* dev)
{
	...
	for (; ; ) {
		...
		{// Packet received from CPU port
			...
			print_packet((void *)KSEG1ADDR(net_rx_packets[rx_dma_owner_idx0]),length);
			inter_loopback_cnt++;
			length = inter_loopback_cnt;
		}
		else {
			net_process_received_packet((uchar *)KSEG1ADDR(net_rx_packets[rx_dma_owner_idx0]), length );
		}
